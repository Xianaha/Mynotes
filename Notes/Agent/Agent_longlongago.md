...
对于记忆，可以按Token（该thing的关键字，special Token）排列成有序队列，为每个Token分配权重，时间越长的Token权重越低，LLM越不容易检索到，时间越短的Token权重越高，LLM越容易检索到。权重低到一定阈值，移除检索的这个库？
也可以加入艾宾浩斯记忆曲线，为每个Token分配生命周期，自动更新。
优点：节省内存，加快检索效率

### Agent finetuning for long-term Memory and Database for short-term Memory

### Agent协同-主动化
参考计算机轮询技术，LLM每隔一段时间检索list，自我提问的方式提出需要干什么以及是否需要对用户提出服务（主动的表现，包装成Request请求？）。list可以包含每日时间表，计划等等，通过与用户接触可以自动向自己的list添加mark（包含时间以及其他因素）。
是否可以通过MCP 调用某个方法使得Agent本身轮询？
...
### Memory editing methods for long-term memory 


### agent concludes its context to reduce the length of context in working